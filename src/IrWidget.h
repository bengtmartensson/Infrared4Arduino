...
public:
    static const int16_t defaultMarkExcess = 0; // signed, thus not microseconds_t
...
#define CAPTURE_PRESCALER_FACTOR (_BV(CAPTURE_PRESCALER_BITS))
    ...
    unsigned int endingTimeout; // = _BV(RANGE_EXTENSION_BITS) - 1;
    static const unsigned int RANGE_EXTENSION_BITS = 4U; // factor for upper measurement range = 2^(RANGE_EXTENSION_BITS+1)
...
protected:
    ...
    size_t captureCount; // number of values stored in captureData
    static const unsigned int numberSavesPerPeriod = 2U;
